...
def choose_input(root):
    ...
    choice = messagebox.askquestion(
        "Input Source",
        "Run from live camera?\n\nClick 'No' for file input.",
        icon="question"
    )
    ...
    if choice == "yes":
        ...
        if ext in ("jpg", "jpeg", "png", "tiff", "bmp"):
            ...
            cap.release()
            cap = None  # not a video stream
            print("Set cap to None")
            print(cap)
            display_image(img)
        else:
            # It's a video
...
def capture_and_detect(counts=999):
    ...
    if cap is not None:
        # Thread for handling a video
        print("looks like we're handling a video")
        ret, frame = cap.read()
        if ret:
            detected_frame = frame.copy()

    else:
        print ("looks like we're handling a single image")
...
source_button = ttk.Button(root, text="Choose Input Source", command=lambda: choose_input(root))
source_button.pack(padx=20, pady=20)
...
if cap:  # camera or video
    print("Starting video feed...")
    update_feed()  # start loop
else:  # single image
